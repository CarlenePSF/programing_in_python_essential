...
# Na linguagem C, temos a função main, que marca o início do programa principal.
...
# Em Python não existe essa estrutura. Se executarmos um módulo diretamente na linha de
...
# Como corrigir? Adicionando a seguinte linha de código no módulo que contém a função chamada.
# if __name__ == '__main__':
#     print('Isso não deveria ser impresso!!!')

# Quando adicionamos a linha acima tudo que estiver no arquivo do código no módulo que contém a função
# utilizada só será impresso se o próprio arquivo-fonte for executado. Isso é muito útil quando queremos
# fazer testes dos métodos ou das funções dos nossos módulos.
